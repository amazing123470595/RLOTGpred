    ...
    def value_estimate(
        ...
        **kwargs,
        ...
        if self.average_rewards:
            reward = reward - reward.mean()
            reward = reward / reward.std().clamp_min(1e-4)
            tensordict.set(("next", self.tensor_keys.reward), reward)

        # 不再使用 next_value，只用 reward 做 target
        # 如果仍然想保留 gamma，也可以写成：value_target = gamma * reward
        value_target = gamma * reward
