...
from Features import GetProtT5_T_4, GetTPEMPPS_CCP, GetESM2, GetOthers  # 假设你有其他两个特征也写好
import pandas as pd
import random
...
actor_net1 = nn.Sequential(
    nn.LazyLinear(X1_train.shape[1], device=device),
    nn.ReLU(),
...
actor_net2 = nn.Sequential(
    nn.LazyLinear(X2_train.shape[1], device=device),
    nn.ReLU(),
    nn.LazyLinear(2, device=device),
)
...
actor_net3 = nn.Sequential(
    nn.LazyLinear(X3_train.shape[1], device=device),
    nn.ReLU(),
    nn.LazyLinear(2, device=device),
)
...
actor_net4 = nn.Sequential(
    nn.LazyLinear(X4_train.shape[1], device=device),
    nn.ReLU(),
    nn.LazyLinear(2, device=device),
)
...
# -----------------------------------------------------生成 stacking 特征-----------------------------------------------------
probs_list2 = []  # shape=(N,4)

with torch.no_grad():
    for i in range(X1_train.shape[0]):
        td1 = TensorDict({"observation": X1_train[i]}, [])
        logits1 = policy_module1(td1)["logits"]
        prob1 = F.softmax(logits1, dim=-1)[1].item()

        td2 = TensorDict({"observation": X2_train[i]}, [])
        logits2 = policy_module2(td2)["logits"]
        prob2 = F.softmax(logits2, dim=-1)[1].item()

        td3 = TensorDict({"observation": X3_train[i]}, [])
        logits3 = policy_module3(td3)["logits"]
        prob3 = F.softmax(logits3, dim=-1)[1].item()

        td4 = TensorDict({"observation": X4_train[i]}, [])
        logits4 = policy_module4(td4)["logits"]
        prob4 = F.softmax(logits4, dim=-1)[1].item()

        probs_list2.append([prob1, prob2, prob3, prob4])

X_meta_train = np.array(probs_list2).astype(np.float32)
...
X_meta_train_tensor = torch.tensor(X_meta_train)
y_train_tensor = torch.tensor(y_train, dtype=torch.long)

# 简单训练几轮
for epoch in range(10000):
    optimizer.zero_grad()
    logits = meta_model(X_meta_train_tensor)
...
torch.save(meta_model.state_dict(), f"4_big_stacking_model_weights_Acc:{Acc:.4f} MCC:{MCC:.4f} Sn:{Sn:.4f} Sp:{Sp:.4f}.pth")
# -----------------------------------------------------保存结果-----------------------------------------------------
